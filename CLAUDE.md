...
- **Killzone calculation** (section 4.8): must use pure UTC arithmetic (`getTimezoneOffset()`) against a stored `locationConfig.offset` — never a named IANA timezone (`Intl.DateTimeFormat` with `America/...`) — since that was the exact bug the v2 rewrite fixed. The offset itself no longer has a UI control (section 4.9 — the "Fuso horário" select was removed for being unreliable); it stays fixed at whatever was last saved. If a UI for it is ever reintroduced, it must keep feeding this same numeric offset, not switch to named-timezone lookup.
