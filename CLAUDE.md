...
- **Killzone calculation** (section 4.8): must use pure UTC arithmetic (`getTimezoneOffset()`) against a user-configurable UTC offset (`locationConfig.offset`, editable via the settings modal — section 4.9/5.9) — never a named IANA timezone (`Intl.DateTimeFormat` with `America/...`) — since that was the exact bug the v2 rewrite fixed. The session table itself (section 4.8) was replaced in this revision — a full 24h coverage table (Pausa/Londres/Londres-NY/NY/Ásia-Tóquio/Pré Sydney) provided in Campo Grande local time and converted to UTC, with one intentional overlapping hour resolved by "last entry listed wins." Don't reintroduce the old table (Ásia/Londres/NY AM/Fechamento Londres/NY PM with unnamed gaps) — it was explicitly replaced, not a bug fix to preserve.

## Storage model

Four keys, each JSON-stringified: `trades-data`, `capital-config`, `filter-config`, `location-config`. `stGet`/`stSet` (`prototype/js/storage.js`) check three layers **on every call**, not once at init — checking once caused a real bug where late host injection made the app silently fall back to non-persistent storage (section 8, bug 3):
1. **Cloud (Firebase Auth + Firestore, `prototype/js/cloud.js`, section 3.1)** — one Firestore doc per authenticated user (`users/{uid}`), one field per key. This is what makes the app usable from more than one device with the same data.
2. `window.storage.get/set` (artifact host API).
3. `localStorage` (final fallback, and always written as a local cache even when the cloud layer succeeds).

The app is gated behind login (`#authOverlay` in `index.html`) — `loadAll()` is triggered by `cloud.js`'s `auth.onAuthStateChanged` callback, not called directly from `init.js` anymore, since `stGet` needs to know the signed-in user before it can decide whether to read from Firestore.

When porting to native platforms, section 9 gives the intended replacement per platform (SQLite/Room/CoreData/JSON file, plus the native Firebase SDK for sync) — the data shapes and migration rules must stay identical regardless of storage backend.
